 ...
 * - $filter      Element status on which to filter
 */
?>
<div class="contentInner even status_<?php echo $elementName; ?>">
  <?php foreach($this->get('canvasItems') as $row) { ?>
    <?php if($row["box"] == "$elementName" && ($row['status'] == $filter || $filter == 'all')) {
      ...
      <div class="ticketBox" id="item_<?php echo $row["id"];?>">
        <div class="row">
          <div class="col-md-12">
            ...
            <h4><a href="<?=BASE_URL ?>/<?=$canvasName ?>canvas/editCanvasItem/<?php echo $row["id"];?>" class="canvasModal"
            ...
            <div class="dropdown ticketDropdown statusDropdown colorized show firstDropdown">
              <a class="dropdown-toggle f-left status label-<?=$statusLabels[$row['status']]['dropdown'] ?>" href="javascript:void(0);" role="button"
			     ...
			     <span class="text"><?php echo $this->__($statusLabels[$row['status']]['title']); ?></span>
              ...
              <ul class="dropdown-menu" aria-labelledby="statusDropdownMenuLink<?=$row['id']?>">
                ...
                <?php foreach($statusLabels as $key => $data) {
                  echo"<li class='dropdown-item'>".
                    "<a href='javascript:void(0);' class='label-".$data['dropdown']."' data-label='".$this->__($data['title'])."' ".
                    "  data-value='".$row['id']."/".$key."' id='ticketStatusChange".$row['id'].$key."'>".$this->__($data['title'])."</a>";
                  echo"</li>";
                }?>
              </ul>
            </div>

			<?php if(!empty($relationalLabels)) { ?>
			   <?php /* TO DO */ ?>
			<?php } ?>
