...
namespace leantime\domain\pdf {
    ...
    class lbmcanvas extends \leantime\domain\pdf\canvas {
        ...
        public function reportGenerate(int $id, array $filter = []): string
        {
            ...
            $lbmCanvasRepo = new repositories\lbmcanvas();
            $lbmCanvasAry = $lbmCanvasRepo->getSingleCanvas($id);
            !empty($lbmCanvasAry) || die("Cannot find canvas with id '$id'");
            $projectId = $lbmCanvasAry[0]['projectId'];
            $recordsAry = $lbmCanvasRepo->getCanvasItemsById($id);
            ...
            // Configuration
            $options = [ 'disclaimer' => $lbmCanvasRepo->getDisclaimer() ];
            
            // Generate PDF content
            $pdf = new \YetiForcePDF\Document();
            $pdf->init();
            $pdf->loadHtml($this->htmlReport($projectAry['name'], $lbmCanvasAry[0]['title'], $recordsAry, $filter, $options));
            return $pdf->render();
