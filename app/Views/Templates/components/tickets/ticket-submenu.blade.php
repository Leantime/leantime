...
@if ($login::userIsAtLeast(\Leantime\Domain\Auth\Models\Roles::$editor))
    ...
    <x-globals::actions.dropdown-menu container-class="pull-right">
            ...
            <li><a href="#/tickets/showTicket/{{ $ticket["id"] }}" class=''><x-globals::elements.icon name="edit" /> {{  __("links.edit_todo") }}</a></li>
            ...
            <li><a  href="javascript:void(0);" onclick="jQuery('#subtask-form-{{$ticket['id']}}').toggle();"
                  ...
                  <x-globals::elements.icon name="arrow_back" /> Add Subtask</a></li>
            @endif
            @dispatchEvent("beforeDeleteTicket", ["ticket"=>$ticket])
            <li><a href="#/tickets/delTicket/{{ $ticket["id"] }}" class="delete"><x-globals::elements.icon name="delete" /> {{  __("links.delete_todo") }}</a></li>
