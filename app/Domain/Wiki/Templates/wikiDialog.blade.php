...
<x-globals::elements.section-title icon="book">{{ __('label.wiki') }} {{ $tpl->escape($currentWiki->title) }}</x-globals::elements.section-title>
    ...
    <div class="row">
        ...
        <div class="col-md-6 align-right padding-top-sm">
            @if (isset($currentWiki->id) && $currentWiki->id != '' && $login::userIsAtLeast($roles::$editor))
                <a href="{{ BASE_URL }}/wiki/delWiki/{{ $currentWiki->id }}" class="delete formModal"><x-globals::elements.icon name="delete" /> {{ __('links.delete_wiki') }}</a>
            @endif
        </div>
    </div>
