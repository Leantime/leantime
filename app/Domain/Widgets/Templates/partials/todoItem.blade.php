...
<div class="sortable-item draggable-todo"
    ...
    >
        @if($hasChildren)
            ...
            >
                <x-globals::elements.icon name="{{ $accordionState == 'closed' ? 'chevron_right' : 'expand_more' }}" />
            </div>
        @endif
        ...
        @if($ticket['type'] == 'milestone')
            <div class="tw:flex tw:flex-row tw:items-center tw:gap-4">
                <div class="tw:flex-grow">
                    <small class="tw:inline-block">{{ $ticket['projectName'] }}</small>
        ...
        @else
            <div class="tw:flex tw:flex-row tw:items-center tw:gap-2">
                ...
                <div class="due-date-container tw:shrink-0 tw:flex tw:flex-row tw:items-center tw:gap-1 due-date-wrapper">
                    <div class="date-picker-form-control">
                        ...
                        <input id="due-date-picker-{{ $ticket['id'] }}"
                               ...
                               class="duedates secretInput tw:ml-0 tw:w-[100px]"
                               data-id="{{ $ticket['id'] }}"
                ...
                <div class="status-container tw:shrink-0 tw:flex tw:flex-row tw:items-center tw:gap-2">
                    @php
                        $statusPatchUrl = BASE_URL . '/widgets/myToDos/updateStatus';
                        $statusHxVals   = json_encode(['id' => (string) $ticket['id']]);
                        $projectStatuses = $statusLabels[$ticket['projectId']] ?? [];
                    @endphp
                    <x-globals::forms.select
                        variant="chip"
                        name="status"
                        :id="'status-chip-' . $ticket['id']"
                    ...
                    >
                        @foreach($projectStatuses as $key => $label)
                            @php
                                $statusName  = is_array($label) ? ($label['name'] ?? $key) : $label;
                                $statusClass = is_array($label) ? ($label['class'] ?? 'label-default') : 'label-default';
                                ...
                                $chipHtml    = '<span class="chip-badge ' . $chipBadgeClass . '" style="' . $chipStyle . '">' . e($statusName) . '</span>';
                                $isSelected  = (string) $ticket['status'] === (string) $key;
                            @endphp
                            <option value="{{ $key }}" {{ $isSelected ? 'selected' : '' }} data-chip-html="{{ $chipHtml }}">{{ $statusName }}</option>
                        @endforeach
                    </x-globals::forms.select>

                    <div class="scheduler">
                        @if( $ticket['editFrom'] != "0000-00-00 00:00:00" && $ticket['editFrom'] != "1969-12-31 00:00:00")
                            <x-globals::elements.icon name="event_available" class="infoIcon" style="color:var(--accent2)"
                               data-tippy-content="{{ __('text.schedule_to_start_on') }} {{ format($ticket['editFrom'])->date() }}" />
                        @else
                            <x-globals::elements.icon name="event_busy" class="infoIcon" data-tippy-content="{{ __('text.not_scheduled_drag_ai') }}" />
                        @endif
                    </div>
    ...
    <!-- Subtask Form -->
    <div id="subtask-form-{{$ticket['id']}}" class="subtask-form tw:m-2 tw:ml-10"
         style="display:none;">
    ...
    <div id="accordion_content-{{ $accordionId }}"
        ...
        @if($level == 0 && $ticket['type'] === "milestone")
            ...
            <!-- Subtask Form -->
            <div id="task-add-form-{{ $groupKey }}-{{$ticket['id']}}" class="subtask-form tw:my-1"
                 style="display:none;">
                <form class="form-group"
            ...
            <a href="javascript:void(0);" id="task-add-form-{{ $groupKey }}-{{$ticket['id']}}-handler"
               onclick="jQuery(this).toggle(); jQuery('#task-add-form-{{ $groupKey }}-{{$ticket['id']}}').toggle(); "><x-globals::elements.icon name="add_circle" /> {{ __('links.add_task') }}</a>
        @endif
