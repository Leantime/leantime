...
@section('content')
    ...
    <script type="text/javascript">
        window.onload = function() {
            if (!window.jQuery) {
                //It's not a modal
                location.href =
                "{{ BASE_URL }}/{{ $canvasName }}canvas/showCanvas?showModal=<?php echo $canvasItem['id']; ?>";
            }
        }
    </script>
    ...
    <div class="" style="width:900px;">
        ...
        <h4 class="widgettitle title-light" style="padding-bottom: 0"><i
                class="fas <?= $canvasTypes[$canvasItem['box']]['icon'] ?>"></i>
            <?= $canvasTypes[$canvasItem['box']]['title'] ?></h4>
        <hr style="margin-top: 5px; margin-bottom: 15px;">
        @displayNotification()

        <form class="formModal" method="post"
            action="{{ BASE_URL }}/{{ $canvasName }}canvas/editCanvasItem/{{ $id }}">
            ...
            <x-global::forms.text-input type="text" name="description"
                value="{{ $tpl->escape($canvasItem['description']) }}" labelText="{{ $tpl->__('label.description') }}"
                class="w-full" />

            @if (!empty($statusLabels))
                <x-global::forms.select name="status" id="statusCanvas" labelText="{!! __('label.status') !!}">
                    @foreach ($statusLabels as $key => $label)
                        <x-global::forms.select.select-option value="{{ $key }}" :selected="$canvasItem['status'] == $key">
                            {!! __($label) !!}
            ...
            @if (!empty($relatesLabels))
                <x-global::forms.select name="relates" id="relatesCanvas" style="width: 50%"
                    labelText="{!! __('label.relates') !!}">
                    @foreach ($relatesLabels as $key => $label)
                        <x-global::forms.select.select-option value="{{ $key }}" :selected="$canvasItem['relates'] == $key">
                            {!! __($label) !!}
                        </x-global::forms.select.select-option>
                    @endforeach
                </x-global::forms.select>
            ...
            @foreach ([1, 2, 3] as $index)
                @if ($dataLabels[$index]['active'])
                    @if (isset($dataLabels[$index]['type']) && $dataLabels[$index]['type'] == 'int')
                        <x-global::forms.text-input type="number" name="{{ $dataLabels[$index]['field'] }}"
                    ...
                    @elseif (isset($dataLabels[$index]['type']) && $dataLabels[$index]['type'] == 'string')
                        <x-global::forms.text-input type="text" name="{{ $dataLabels[$index]['field'] }}"
                            value="{{ $canvasItem[$dataLabels[$index]['field']] }}"
                            labelText="{{ $tpl->__($dataLabels[$index]['title']) }}" class="w-full" />
                    @else
            ...
            @if ($id != '')
                <a href="{{ BASE_URL }}/{{ $canvasName }}canvas/delCanvasItem/{{ $id }}"
                    class="{{ $canvasName }}CanvasModal delete right">
            ...
            @if ($login::userIsAtLeast($roles::$editor))
                ...
                <x-global::forms.button type="submit" id="saveAndClose"
            ...
            <center>
                ...
                <div class="row" id="milestoneSelectors">
                    ...
                    <div class="col-md-12">
                        <a href="javascript:void(0);"
                            onclick="leantime.canvasController.toggleMilestoneSelectors('new');"><?= $tpl->__('links.create_link_milestone') ?></a>
                        <?php if (count($tpl->get('milestones')) > 0) { ?>
                        | <a href="javascript:void(0);"
                            onclick="leantime.canvasController.toggleMilestoneSelectors('existing');"><?= $tpl->__('links.link_existing_milestone') ?></a>
                        <?php } ?>
                    </div>
                    <?php } ?>
                </div>
                <div class="row" id="newMilestone" style="display:none;">
                    <div class="col-md-12">
                        ...
                        <x-global::forms.button type="button" contentRole="secondary"
                ...
                <div class="row" id="existingMilestone" style="display:none;">
                    <div class="col-md-12">
                        <x-global::forms.select name="existingMilestone" class="user-select" :placeholder="__('input.placeholders.filter_by_milestone')">
                        ...
                        <input type="hidden" name="type" value="milestone" />
                        <input type="hidden" name="{{ $canvasName }}canvasitemid" value="<?php echo $id; ?> " />
                        <input type="button" value="<?= $tpl->__('buttons.save') ?>"
                            onclick="jQuery('#primaryCanvasSubmitButton').click()" class="btn btn-primary" />
                        <input type="button" value="<?= $tpl->__('buttons.cancel') ?>"
                            onclick="leantime.canvasController.toggleMilestoneSelectors('hide')"
            ...
            <a href="<?= CURRENT_URL ?>?removeMilestone=<?php echo $canvasItem['milestoneId']; ?>"
