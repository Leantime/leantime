...
@section('content')
    ...
    <form
        action="{{ BASE_URL }}/<?= $canvasName ?>canvas/boardDialog<?= isset($_GET['id']) ? '/' . (int) $_GET['id'] : '' ?>"
        method="post" class="formModal">
        ...
        <div class="modal-body">
            ...
            <x-global::forms.text-input type="text" name="canvastitle" value="{{ $canvasTitle }}"
                placeholder="{{ $tpl->__('input.placeholders.enter_title_for_board') }}" variant="title" />
