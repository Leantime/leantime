...
<x-globals::layout.page-header icon="luggage" subtitle="{{ __('label.administration') }}" headline="{{ sprintf(__('headline.project'), e($project['name'])) }}" />
...
<div class="maincontent">
    <div class="maincontentinner">
        ...
        <div class="inlineDropDownContainer tw:float-right tw:z-9 tw:pt-1">
        ...
        <x-globals::navigations.tabs persist="url">
            <x-slot:headings>
                <x-globals::navigations.tabs.heading name="projectdetails"><x-globals::elements.icon name="eco" /> {{ __('tabs.projectdetails') }}</x-globals::navigations.tabs.heading>
                <x-globals::navigations.tabs.heading name="team"><x-globals::elements.icon name="group" /> {{ __('tabs.team') }}</x-globals::navigations.tabs.heading>
                <x-globals::navigations.tabs.heading name="integrations"><x-globals::elements.icon name="emergency" /> {{ __('tabs.Integrations') }}</x-globals::navigations.tabs.heading>
                <x-globals::navigations.tabs.heading name="todosettings"><x-globals::elements.icon name="format_list_bulleted" /> {{ __('tabs.todosettings') }}</x-globals::navigations.tabs.heading>
            ...
            <x-slot:contents>
                ...
                <x-globals::navigations.tabs.content name="team">
                    <form method="post" action="{{ BASE_URL }}/projects/showProject/{{ $project['id'] }}#team">
                        <input type="hidden" name="saveUsers" value="1" />

                        <p class="tw:mb-6 tw:opacity-80" style="color: var(--primary-font-color);">
                            {!! __('text.choose_access_for_users') !!}
                        </p>

                        <x-globals::elements.section-title icon="group">{{ __('headlines.team_member') }}</x-globals::elements.section-title>
                                ...
                                <div class="col-md-4">
                                    <div class="maincontentinner">
                                        ...
                                        @if($roles::getRoles()[$assignedUser['role']] == $roles::$admin || $roles::getRoles()[$assignedUser['role']] == $roles::$owner)
                                            <x-globals::forms.text-input :bare="true" name="role-{{ $assignedUser['id'] }}" readonly disabled value="{{ __('label.roles.' . $roles::getRoles()[$assignedUser['role']]) }}" />
                                        @else
                        ...
                        <x-globals::elements.section-title icon="group" class="tw:mt-10">{{ __('headlines.assign_users_to_project') }}</x-globals::elements.section-title>
                        ...
                        <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="maincontentinner">
                                            ...
                                            @if($roles::getRoles()[$row['role']] == $roles::$admin || $roles::getRoles()[$row['role']] == $roles::$owner)
                                                <x-globals::forms.text-input :bare="true" name="role-{{ $row['id'] }}" readonly disabled value="{{ __('label.roles.' . $roles::getRoles()[$row['role']]) }}" />
                                            @else
                            ...
                            @if($login::userIsAtLeast($roles::$manager))
                                <div class="col-md-4">
                                    <div class="maincontentinner userBox--add">
                                        <a class="userEditModal" href="{{ BASE_URL }}/users/newUser?preSelectProjectId={{ $project['id'] }}"><x-globals::elements.icon name="person_add" /> {{ __('links.create_user') }}</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                <x-globals::navigations.tabs.content name="integrations">

                    <x-globals::elements.section-title icon="eco">Mattermost</x-globals::elements.section-title>
                    <div class="row">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <form action="{{ BASE_URL }}/projects/showProject/{{ $project['id'] }}#integrations" method="post">
                                <x-globals::forms.text-input name="mattermostWebhookURL" id="mattermostWebhookURL" value="{{ e($tpl->get('mattermostWebhookURL')) }}" />
                                <br />
                    ...
                    <br />
                    <x-globals::elements.section-title icon="eco">Slack</x-globals::elements.section-title>
                    <div class="row">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-4">
                            <strong>{{ __('label.webhook_url') }}</strong><br />
                            <form action="{{ BASE_URL }}/projects/showProject/{{ $project['id'] }}#integrations" method="post">
                                <x-globals::forms.text-input name="slackWebhookURL" id="slackWebhookURL" value="{{ e($tpl->get('slackWebhookURL')) }}" />
                                <br />
                    ...
                    <x-globals::elements.section-title icon="eco">Zulip</x-globals::elements.section-title>
                    <div class="row">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-4">
                            <form action="{{ BASE_URL }}/projects/showProject/{{ $project['id'] }}#integrations" method="post">
                                ...
                                <x-globals::forms.text-input name="zulipURL" id="zulipURL" placeholder="{{ __('input.placeholders.zulip_url') }}" value="{{ $tpl->get('zulipHook')['zulipURL'] }}" />
                                <br />
                                <strong>{{ __('label.bot_email') }}</strong><br />
                                <x-globals::forms.text-input name="zulipEmail" id="zulipEmail" value="{{ e($tpl->get('zulipHook')['zulipEmail']) }}" />
                                <br />
                                <strong>{{ __('label.botkey') }}</strong><br />
                                <x-globals::forms.text-input name="zulipBotKey" id="zulipBotKey" value="{{ e($tpl->get('zulipHook')['zulipBotKey']) }}" />
                                <br />
                                <strong>{{ __('label.stream') }}</strong><br />
                                <x-globals::forms.text-input name="zulipStream" id="zulipStream" value="{{ e($tpl->get('zulipHook')['zulipStream']) }}" />
                                <br />
                                <strong>{{ __('label.topic') }}</strong><br />
                                <x-globals::forms.text-input name="zulipTopic" id="zulipTopic" value="{{ e($tpl->get('zulipHook')['zulipTopic']) }}" />
                                <br />
                    ...
                    <x-globals::elements.section-title icon="eco">Discord</x-globals::elements.section-title>
                    <div class='row'>
                        <div class='col-md-3'>
                        ...
                        <div class="col-md-4">
                            ...
                            <form action="{{ BASE_URL }}/projects/showProject/{{ $project['id'] }}#integrations" method="post">
                                @for($i = 1; $i <= 3; $i++)
                                <x-globals::forms.text-input name="discordWebhookURL{{ $i }}" id="discordWebhookURL{{ $i }}" placeholder="{{ __('input.placeholders.discord_url') }}" value="{{ e($tpl->get('discordWebhookURL' . $i)) }}" /><br/>
                                @endfor
                ...
                <x-globals::navigations.tabs.content name="todosettings">
                    <form action="{{ BASE_URL }}/projects/showProject/{{ $project['id'] }}#todosettings" method="post">
                                    ...
                                    <div class="ticketBox">
                                        ...
                                        <div class="row statusList" id="todostatus-{{ $key }}">
                                            ...
                                            <div class="sortHandle">
                                                <x-globals::elements.icon name="sort" />
                                            </div>
                                            <div class="col-md-1">
                                                <x-globals::forms.text-input :bare="true" type="text" name="labelSort-{{ $key }}" id="labelSort-{{ $key }}" value="{{ e($ticketStatus['sortKey']) }}" class="sorter tw:w-12" />
                                            </div>
                                            <div class="col-md-3">
                                            ...
                                            <div class="remove">
                                                @if($key != -1)
                                                    <a href="javascript:void(0);" onclick="leantime.projectsController.removeStatus({{ $key }})" class="delete" aria-label="{{ __('label.remove') }}"><x-globals::elements.icon name="delete" /></a>
                                                @endif
                                            </div>
                                        </div>
                        ...
                        <a href="javascript:void(0);" onclick="leantime.projectsController.addToDoStatus();" class="quickAddLink" class="tw:text-left">{!! __('links.add_status') !!}</a>
                        <br />
...
<!-- New Status Template -->
<div class="newStatusTpl tw:hidden">
    <div class="ticketBox">
    <div class="row statusList" id="todostatus-XXNEWKEYXX">
        ...
        <div class="col-md-1">
            <x-globals::forms.text-input :bare="true" type="text" name="labelSort-XXNEWKEYXX" id="labelSort-XXNEWKEYXX" value="" class="sorter tw:w-12" />
        </div>
        <div class="col-md-3">
        ...
        <div class="remove">
            <a href="javascript:void(0);" onclick="leantime.projectsController.removeStatus('XXNEWKEYXX')" class="delete" aria-label="{{ __('label.remove') }}"><x-globals::elements.icon name="delete" /></a>
        </div>
    </div>
</div>
</div>
