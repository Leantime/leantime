...
@section('content')
    ...
    <form method="post" accept-charset="utf-8" action="<?php echo $formUrl; ?>" id="commentForm-<?= $formHash ?>" class="formModal">
        ...
        <div id="comment-<?= $formHash ?>-0" class="commentBox-<?= $formHash ?> commenterFields" style="display:none;">
            ...
            <div class="commentReply">
                ...
                <input type="submit" value="<?php echo $tpl->__('buttons.save'); ?>" name="comment" class="btn btn-primary btn-success"
            ...
            <input type="hidden" name="edit-comment-helper" class="commenterField"
                id="edit-comment-helper-<?= $formHash ?>" />
            <br />
        </div>
        <?php } ?>
            ...
            <div>
                ...
                <div class="clearall">
                    <div class="commentImage" id="comment-image-to-hide-on-edit-<?= $formHash ?>-<?= $row['id'] ?>">
                        <img
                            src="<?= BASE_URL ?>/api/users?profileImage=<?= $row['userId'] ?>&v=<?= format($row['userModified'])->timestamp() ?>" />
                    </div>
                    <div class="commentMain">
                        <div class="commentContent" id="comment-to-hide-on-edit-<?= $formHash ?>-<?= $row['id'] ?>">
                            <div class="right commentDate">
                                <?php printf($tpl->__('text.written_on'), format($row['date'])->date(), format($row['date'])->time()); ?>
                                <?php if ($login::userIsAtLeast($roles::$editor)) { ?>
                                <x-global::actions.dropdown label-text="<i class='fa fa-ellipsis-v' aria-hidden='true'></i>"
                                    contentRole="link" position="bottom" align="start" class="inlineDropDownContainer"
                                    ...
                                    <x-slot:menu>
                                        ...
                                        @if ($row['userId'] == session('userdata.id') || $login::userIsAtLeast($roles::$manager))
                                            <x-global::actions.dropdown.item variant="link"
                                                href="{{ $deleteUrlBase . $row['id'] }}" class="deleteComment formModal">
                                                <span class="fa fa-trash"></span> {{ __('links.delete') }}
                                            </x-global::actions.dropdown.item>
                                        @endif

                                        <!-- Conditional Edit Menu Item -->
                                        @if ($row['userId'] == session('userdata.id') || $login::userIsAtLeast($roles::$manager))
                                            <x-global::actions.dropdown.item variant="link" href="javascript:void(0);"
                                                onclick="toggleCommentBoxes({{ $row['id'] }}, null, '{{ $formHash }}', true)">
                                                <span class="fa fa-edit"></span> {{ __('label.edit') }}
                                            </x-global::actions.dropdown.item>
                                        @endif

                                        <!-- Add to Timesheets Menu Item -->
                                        @if (isset($tpl->get('ticket')->id))
                                            <x-global::actions.dropdown.item variant="link" href="javascript:void(0);"
                                                onclick="leantime.ticketsController.addCommentTimesheetContent({{ $row['id'] }}, {{ $tpl->get('ticket')->id }});">
                        ...
                        <div class="replies">
                            ...
                            <div>
                                <div class="commentImage">
                                    <img
                                        src="<?= BASE_URL ?>/api/users?profileImage=<?= $comment['userId'] ?>&v=<?= format($comment['userModified'])->timestamp() ?>" />
                                </div>
                                <div class="commentMain">
                                    <div class="commentContent">
                                        <div class="right commentDate">
                                            <?php printf($tpl->__('text.written_on'), format($comment['date'])->date(), format($comment['date'])->time()); ?>
                                        </div>
                                        ...
                                        <div class="text mce-content-body"
                                            id="comment-text-to-hide-reply-<?= $formHash ?>-<?= $comment['id'] ?>">
                                            <?php echo $tpl->escapeMinimal($comment['text']); ?></div>
                                    </div>
                                    ...
                                    <div class="commentLinks">
                                        ...
                                        <a href="<?php echo $deleteUrlBase . $comment['id']; ?>" class="deleteComment formModal">
                                            <span class="fa fa-trash"></span> <?php echo $tpl->__('links.delete'); ?>
                                        </a>
                                        <a href="javascript:void(0);"
                                            onclick="toggleCommentBoxes(<?php echo $row['id']; ?>, <?= $comment['id'] ?>, '<?= $formHash ?>', true, true)">
                            ...
                            <?php endforeach; ?>
                            <?php endif; ?>
                            <div style="display:none;" id="comment-<?= $formHash ?>-<?php echo $row['id']; ?>"
                                class="commentBox">
                                <div class="commentImage">
                                    <img
                                        src="<?= BASE_URL ?>/api/users?profileImage=<?= session('userdata.id') ?>&v=<?= format(session('userdata.modified'))->timestamp() ?>" />
                                ...
                                <div class="commentReply">
                                    <input type="submit" value="<?php echo $tpl->__('links.reply'); ?>" name="comment"
                                        id="submit-reply-button" class="btn btn-primary" />
                                    <input type="button" onclick="cancel(<?php echo $row['id']; ?>, '<?= $formHash ?>')"
                                        value="<?php echo $tpl->__('links.cancel'); ?>" class="btn btn-primary" />
                                </div>
                                <div class="clearall"></div>
                            </div>
                        </div>
    ...
    <script type='text/javascript'>
        ...
        function toggleCommentBoxes(id, commentId, formHash, editComment = false, isReply = false) {
            ...
            jQuery(`#comment-${formHash}-${id} .commentReply`).prepend(
                `<textarea rows="5" cols="75" name="text" id="editor_${formHash}-${id}" class="tinymceSimple">${editComment ? jQuery(`#comment-text-to-hide-${isReply ? 'reply-' : ''}${formHash}-${commentId || id}`).html() : ''}</textarea>`
                );
            leantime.editorController.initSimpleEditor();
