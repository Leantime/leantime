...
if (isset($_SESSION['userdata']['settings']['views']['programsTimeline'])) {
    $roadmapView = $_SESSION['userdata']['settings']['views']['programsTimeline'];
} else {
    $roadmapView = "Month";
}
?>
<div class="pageheader">
    ...
    <div class="pagetitle">
        ...
        <h1>Program: <?=$_SESSION['currentProjectName'] ?></h1>
    </div>
</div><!--pageheader-->
    ...
    <?php if (count($programs) > 0) {?>
        var tasks = [
            ...
            <?php foreach ($programs as $program) {
                ...
                //if($program->sortIndex != '' && is_numeric($program->sortIndex)){
                ...
                echo"{
                    ...
                    start :'" . (($program->start != '0000-00-00 00:00:00' && substr($program->start, 0, 10) != '1969-12-31' && $program->start != '') ? $program->start :  date('Y-m-d', strtotime("+1 day", time()))) . "',
                    end :'" . (($program->end != '0000-00-00 00:00:00' && substr($program->end, 0, 10) != '1969-12-31' && $program->end != '') ? $program->end :  date('Y-m-d', strtotime("+1 week", time()))) . "',
                    progress :'0',
                    dependencies :'" . implode(",", $dependencyList) . "',
                    custom_class :'',
                    type: '" . strtolower($program->type) . "',
                    bg_color: '" . $color . "',
                    thumbnail: '" . BASE_URL . "/api/projects?projectAvatar=" . $program->id . "',
                    sortIndex: ".$program->sortIndex."
        ...
        leantime.projectsController.initGanttChart(tasks, '<?=$roadmapView; ?>', false);
        <?php } else { ?>
        leantime.projectsController.initGanttChart(tasks, '<?=$roadmapView; ?>', true);
        <?php } ?>
