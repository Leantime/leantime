    ...
    class newProgram extends controller
    {
        ...
        public function run()
        {
            ...
            if (!isset($_SESSION['lastPage'])) {
                $_SESSION['lastPage'] = BASE_URL . "/programs/showAll";
            }
            ...
            if (isset($_POST['save']) === true) {
                ...
                if ($values['name'] === '') {
                    ...
                    $mailer->setContext('program_created');
                    $mailer->setSubject($this->language->__('email_notifications.program_created_subject'));
                    $actual_link = BASE_URL . "/projects/showProject/" . $id . "";
                    $message = sprintf($this->language->__('email_notifications.program_created_message'), $actual_link, $id, $projectName, $_SESSION["userdata"]["name"]);
                    ...
                    if ($values['menuType'] == 'dts') {
                        $this->tpl->setNotification(sprintf($this->language->__('notifications.program_created_successfully'), BASE_URL . '/lbmcanvas/showCanvas/'), 'success');
                    } else {
                        $this->tpl->setNotification(sprintf($this->language->__('notifications.program_created_successfully'), BASE_URL . '/leancanvas/simpleCanvas/'), 'success');
                    }

                    $this->tpl->redirect(BASE_URL . "/pgmPro/showProgram/" . $id);
                }


                $this->tpl->assign('program', $values);
            }
            ...
            $this->tpl->display('pgmPro.newProgram');
        }
    }
