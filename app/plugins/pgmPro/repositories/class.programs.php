...
namespace leantime\plugins\repositories\pgmPro {
...
";
         ...
         * @return array
         */
        public function getAllProgramProjects($programId)
        {
            ...
            $query = "SELECT
				...
				LEFT JOIN zp_clients as client ON project.clientId = client.id
				LEFT JOIN zp_tickets as ticket ON project.id = ticket.projectId
				WHERE project.parent = :programId";
            ...
            $stmn = $this->db->database->prepare($query);
            $stmn->bindValue(':programId', $programId, PDO::PARAM_INT);

            $stmn->execute();
            $stmn->setFetchMode(PDO::FETCH_CLASS, "\leantime\domain\models\projects\project");
            $values = $stmn->fetchAll();
            $stmn->closeCursor();
        ...
        public function getUserPrograms($userId, $status = "all", $clientId = "")
        {
