...
<form method="post" accept-charset="utf-8" action="index.php?act=tickets.showTicket&amp;id=<?php echo $ticket['id']; ?>#commentList">

	<br /><br />

	<spanid="comment0" class="commentBox">
...
<?php foreach($this->get('comments') as $row) { ?>
	...
	<span style="display:block; padding-left:10px; margin-left:<?php echo $tabs;?>px; <?php if($tabs > 1) echo'background:#e1e1e1;'?> border-bottom:1px solid #fff;">
		...
		|<a href="javascript:void(0);" onclick="toggleCommentBoxes(<?php echo $k; ?>)"><?php echo $language->lang_echo('COMMENT') ?></a>
		<br /><br /><hr />
