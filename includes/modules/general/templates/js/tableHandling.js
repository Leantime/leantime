...
(function($) {
	...
	$(document).ready(function() 
		...
		if ( Get_Cookie( 'pagerSize' ) ) {				
			...
			var size = Get_Cookie('pagerSize');			
			
		}else{
		
			var size = setDefaultPagerCookie(size);
		...
		oTable = $("#resultTable").dataTable({
        	   ...
        	   "bFilter": true,
        	   "sDom": '<"H"<"left"T><"left"l><"right"C><"right"f>>rt<"F"<"left"l><"left"i><"right"p>>',
        	   "iDisplayLength": size,
        	   ...
        	   "oLanguage": {
					...
					"sInfoEmpty": "Show 0 to 0 of 0 pages",
					"sInfoFiltered": "(Filtered of _MAX_ entries)"
       			}
        	   });

			$("#resultTable thead tr.filter th:not(.noFilter)").each( function ( i ) {
			...
			$('.dataTables_length select').change(function(){
				...
				var newSize = $('#resultTable_length select option:selected').val();

				setPagerCookie(newSize);
			...
			var oTable2 = $("#resultTable2").dataTable({
	        	   ...
	        	   "sPaginationType": "full_numbers",
	        	   "bFilter": true,
	        	   "sDom": '<"H"<"left"l><"right"f>>rt<"F"<"left"l><"left"i><"right"p>>',
        	   		"iDisplayLength": size,
		       	   "oTableTools": {
	        		    "sSwfPath": "includes/libs/DataTables-1.7.6/extras/TableTools/media/swf/copy_cvs_xls_pdf.swf",
	        		    "aButtons": [
				...
				$("#resultTable2 thead tr.filter th").each( function ( i ) {
					this.innerHTML = fnCreateSelect( oTable2.fnGetColumnData(i) );
					$('select', this).change( function () {
						oTable2.fnFilter( $(this).val(), i );
					} );
				});
			
				$('.dataTables_length select').change(function(){
					var newSize = $('.dataTables_length select option:selected').val();
