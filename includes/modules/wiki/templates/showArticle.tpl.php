...
function toggleCommentBoxes(id){
		...
		$('.commentBox').hide('fast',function(){
			...
			$('#comment'+id+'').prepend('<textarea rows="5" cols="50" name="text"></textarea>');
...
<p>
	...
	<a href="index.php?act=wiki.editArticle&id=<?php echo $article['id']; ?>"><?php echo $lang['EDIT']; ?></a>
	<?php } ?> 
	
	<?php if($this->get('role') === 'admin'){ ?> | <a
	href="index.php?act=wiki.delArticle&id=<?php echo $article['id']; ?>"><?php echo $lang['DELETE']; ?></a>
	<?php } ?></p>
</div>
</div>
